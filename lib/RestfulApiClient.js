!function() {
	...
	var APIClient = module.exports = new Class({
		...
		, init: function(definition) {
            ...
            if (type.number(definition.interval)) this.interval = definition.interval;
        ...
        , handleListRequest: function(request, callback) {
            ...
            this.limiter.request(request, function(err, response) {
                if (debug) {
                    this.requestCount++;
                    log.debug('Fired %s requests, %s requests per minute ...', this.requestCount, (this.requestCount/((Date.now()-this.firstRequest)/1000/this.interval)));
                }
                if (err) {
